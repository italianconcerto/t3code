...
// Exercise the common orchestration contract for every built-in driver. This
// matrix uses a mocked ProviderService; it does not prove native MCP delivery.
const automationModels = [
  { instanceId: "codex", model: "gpt-5-codex" },
  { instanceId: "claudeAgent", model: "claude-sonnet-4" },
  { instanceId: "cursor", model: "auto" },
  { instanceId: "grok", model: "grok-code-fast-1" },
  { instanceId: "opencode", model: "opencode/test-model" },
  { instanceId: "antigravity", model: "test-model" },
  { instanceId: "openrouter", model: "openrouter/test/model" },
].map(({ instanceId, model }) => ({ instanceId: ProviderInstanceId.make(instanceId), model }));
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    readonly beforeGoalContinuationDispatch?: () => Effect.Effect<void>;
    ...
    const sendTurn = vi.fn((request: unknown) => {
      ...
      const requestedModelSelection =
        typeof request === "object" && request !== null && "modelSelection" in request
          ? (request.modelSelection as ModelSelection | undefined)
          : undefined;
      const providerInstanceId = requestedModelSelection?.instanceId ?? modelSelection.instanceId;
      const terminal: ProviderRuntimeEvent = {
        type: "turn.completed",
        eventId: EventId.make("early-goal-terminal"),
        provider: ProviderDriverKind.make(providerInstanceId),
        providerInstanceId,
        threadId: ThreadId.make("thread-1"),
      ...
      const staleTerminal: ProviderRuntimeEvent = {
        ...terminal,
        eventId: EventId.make("stale-goal-terminal"),
        turnId: asTurnId("stale-turn"),
      };
      return PubSub.publish(runtimeEventPubSub, terminal).pipe(
        Effect.andThen(
          input.staleTerminalAfterEarly
            ? PubSub.publish(runtimeEventPubSub, staleTerminal)
            : Effect.void,
        ),
        Effect.as(result),
      );
    ...
    return {
      ...
      readManagedGoal: (threadId = ThreadId.make("thread-1")) =>
        runtime!.runPromise(
          Effect.gen(function* () {
            const sql = yield* SqlClient.SqlClient;
      ...
      setManagedGoalStatus: (status: "complete" | "blocked") =>
        runtime!.runPromise(
          Effect.gen(function* () {
            const sql = yield* SqlClient.SqlClient;
            yield* sql`UPDATE managed_goals SET status = ${status}, awaiting_turn = 0 WHERE thread_id = 'thread-1'`;
          }),
        ),
      ...
      dropManagedGoalsTable: () =>
        runtime!.runPromise(
          Effect.gen(function* () {
            const sql = yield* SqlClient.SqlClient;
            yield* sql`DROP TABLE managed_goals`;
          }),
        ),
      ...
      emitRuntimeEvent: (event: ProviderRuntimeEvent) =>
        runtime!.runPromise(PubSub.publish(runtimeEventPubSub, event)),
  ...
  it("starts a provider-independent persistent goal", async () => {
    const harness = await createHarness();
    await dispatchAutomation(harness, "/goal --budget 5000 Finish migration", "goal-command");
    await harness.drain();
  ...
  it.each(automationModels)(
    "$instanceId reports, pauses, resumes, and clears a persistent goal without provider prompts",
    async (threadModelSelection) => {
      const harness = await createHarness({ threadModelSelection });
      await dispatchAutomation(harness, "/goal Finish migration", "goal-set");
      await harness.drain();
      for (const [index, command] of ["status", "pause", "resume", "clear", "status"].entries()) {
        await dispatchAutomation(harness, `/goal ${command}`, `goal-control-${index}`);
        await harness.drain();
      }
      expect(harness.goal).not.toHaveBeenCalled();
      expect(harness.sendTurn).toHaveBeenCalledOnce();
      expect((await harness.readModel()).threads[0]?.activities).toContainEqual(
        expect.objectContaining({ summary: "No active goal." }),
      );
    },
  );

  it.each(["complete", "blocked"] as const)(
    "does not overwrite concurrent %s after continuation dispatch fails",
    async (status) => {
      const entered = Deferred.makeUnsafe<void>();
      const release = Deferred.makeUnsafe<void>();
      const harness = await createHarness({
        testClock: true,
        beforeGoalContinuationDispatch: () =>
          Effect.gen(function* () {
            yield* Deferred.succeed(entered, undefined);
            yield* Deferred.await(release);
            return yield* Effect.die("Injected continuation dispatch failure");
          }),
      });
      const threadId = ThreadId.make("thread-1");
      await dispatchAutomation(harness, "/goal Finish it", "race-goal-set");
      await harness.drain();
      await harness.emitRuntimeEvent({
        type: "turn.completed",
        eventId: EventId.make("race-goal-terminal"),
      ...
      await harness.runEffect(
        harness.engine.dispatch({
          type: "thread.session.set",
          commandId: CommandId.make("race-session-ready"),
          threadId,
          session: {
            threadId,
            status: "ready",
      ...
      await harness.drain();
      const tick = harness.runEffect(TestClock.adjust("1 second"));
      await harness.runEffect(Deferred.await(entered));
      await harness.setManagedGoalStatus(status);
      await harness.runEffect(Deferred.succeed(release, undefined));
      await tick;
      await harness.drain();
      expect((await harness.readManagedGoal())?.status).toBe(status);
      expect(
        (await harness.readModel()).threads[0]?.activities.some(
          (activity) => activity.summary === "Goal blocked after continuation dispatch failed",
        ),
      ).toBe(false);
    },
  );

  it("invalidates old tool reports on resume without losing in-flight turn usage", async () => {
    const harness = await createHarness({ testClock: true });
    await dispatchAutomation(harness, "/goal Finish it", "resume-accounting-set");
    await harness.drain();
    const before = await harness.readManagedGoal();
    expect(before?.expectedTurnId).toBe("turn-1");
    await dispatchAutomation(harness, "/goal pause", "resume-accounting-pause");
    await harness.drain();
    await dispatchAutomation(harness, "/goal resume", "resume-accounting-resume");
    await harness.drain();
    const resumed = await harness.readManagedGoal();
    expect(resumed?.goalId).toBe(before?.goalId);
    expect(resumed?.turnNumber).toBe((before?.turnNumber ?? 0) + 1);
    expect(resumed?.expectedTurnId).toBe("turn-1");
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: EventId.make("resume-accounting-terminal"),
      provider: ProviderDriverKind.make("codex"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      threadId: ThreadId.make("thread-1"),
      turnId: asTurnId("turn-1"),
  ...
  it("runs managed goals on providers without a native goal API", async () => {
    const harness = await createHarness({
    ...
    expect(harness.sendTurn).toHaveBeenCalledWith(
      expect.objectContaining({
        input: expect.stringContaining("T3 Code MCP tool t3_update_goal"),
      }),
    );
    expect(await harness.readPendingTurnStarts()).toEqual([]);
  });
  ...
  it.each(automationModels)(
    "$instanceId continues an active goal after a successful provider turn",
    async (threadModelSelection) => {
      const harness = await createHarness({ testClock: true, threadModelSelection });
      const threadId = ThreadId.make("thread-1");
      await dispatchAutomation(harness, "/goal Finish it", "goal-autocontinue");
      await harness.drain();
      expect(harness.sendTurn).toHaveBeenCalledOnce();

      await harness.emitRuntimeEvent({
        type: "turn.completed",
        eventId: EventId.make("goal-turn-completed"),
        provider: ProviderDriverKind.make(threadModelSelection.instanceId),
        providerInstanceId: threadModelSelection.instanceId,
        threadId,
        turnId: asTurnId("turn-1"),
        createdAt: "2026-01-01T00:00:02.000Z",
        payload: {
      ...
      await harness.runEffect(
        harness.engine.dispatch({
          type: "thread.session.set",
          commandId: CommandId.make("goal-session-ready"),
          threadId,
          session: {
            threadId,
            status: "ready",
            providerName: threadModelSelection.instanceId,
            providerInstanceId: threadModelSelection.instanceId,
            runtimeMode: "approval-required",
            activeTurnId: null,
            lastError: null,
  ...
  it("processes a goal terminal emitted before sendTurn returns", async () => {
    const harness = await createHarness({
      terminalBeforeSendTurnReturns: true,
      staleTerminalAfterEarly: true,
    });
    await dispatchAutomation(harness, "/goal Finish it", "goal-early-terminal");
    await waitFor(async () => (await harness.readManagedGoal())?.awaitingTurn === false);
    await harness.drain();
  ...
  it("stops automatic continuation when the goal token budget is reached", async () => {
    const harness = await createHarness({ testClock: true });
    const threadId = ThreadId.make("thread-1");
    await dispatchAutomation(harness, "/goal --budget 12 Finish it", "goal-budget");
    await harness.drain();
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: EventId.make("goal-budget-completed"),
      provider: ProviderDriverKind.make("codex"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      threadId,
      turnId: asTurnId("turn-1"),
    ...
    await harness.drain();
    await harness.runEffect(TestClock.adjust("2 seconds"));
    await harness.drain();

    expect(harness.sendTurn).toHaveBeenCalledOnce();
    expect((await harness.readModel()).threads[0]?.activities).toContainEqual(
      expect.objectContaining({ summary: expect.stringContaining("Goal budgetLimited") }),
    );
  });

  it("ignores stale-provider and duplicate terminal events for a goal turn", async () => {
    const harness = await createHarness();
    const threadId = ThreadId.make("thread-1");
    await dispatchAutomation(harness, "/goal Finish it", "goal-correlated");
    await harness.drain();

    const terminal = {
      type: "turn.completed" as const,
      eventId: EventId.make("goal-correlated-terminal"),
      provider: ProviderDriverKind.make("codex"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      threadId,
      turnId: asTurnId("turn-1"),
    ...
    await harness.emitRuntimeEvent({
      ...terminal,
      eventId: EventId.make("goal-stale-provider-terminal"),
      provider: ProviderDriverKind.make("claudeAgent"),
      providerInstanceId: ProviderInstanceId.make("claudeAgent_gradient_6"),
    });
    await harness.drain();
    expect(await harness.readManagedGoal()).toMatchObject({
      tokensUsed: 0,
  ...
  it("keeps a managed goal authoritative after switching to Codex", async () => {
    ...
    const state = await harness.readModel();
    expect(state.threads[0]?.activities).toContainEqual(
      expect.objectContaining({ summary: expect.stringContaining("Goal active: Finish it") }),
    );
  });

  it("correlates a managed goal with a replacement subscription", async () => {
    const originalInstanceId = ProviderInstanceId.make("claudeAgent");
    const replacementInstanceId = ProviderInstanceId.make("claudeAgent_gradient_6");
    const harness = await createHarness({
      threadModelSelection: {
    ...
    await dispatchAutomation(harness, "/goal Finish it", "goal-before-subscription-switch");
    await harness.drain();
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: EventId.make("goal-original-subscription-terminal"),
      provider: ProviderDriverKind.make("claudeAgent"),
    ...
    await harness.runEffect(
      harness.engine.dispatch({
        ...commandInput("Continue with the replacement subscription", "goal-switch-subscription"),
        modelSelection: {
    ...
    await waitFor(async () => (await harness.readManagedGoal())?.expectedTurnId !== null);
    await harness.drain();

    expect(await harness.readManagedGoal()).toMatchObject({
      awaitingTurn: true,
      expectedProviderInstanceId: replacementInstanceId,
      expectedTurnId: asTurnId("turn-1"),
    });
  });

  it("keeps the shared worker alive after a goal runtime persistence failure", async () => {
    const harness = await createHarness();
    await harness.dropManagedGoalsTable();
    await harness.emitRuntimeEvent({
      type: "turn.completed",
      eventId: EventId.make("goal-runtime-storage-failure"),
      provider: ProviderDriverKind.make("codex"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      threadId: ThreadId.make("thread-1"),
      turnId: asTurnId("turn-1"),
    ...
    await dispatchAutomation(harness, "/loop status", "loop-status-after-goal-failure");
    await harness.drain();
    expect((await harness.readModel()).threads[0]?.activities).toContainEqual(
      expect.objectContaining({ summary: "No active loop." }),
    );
  });

  it.each(automationModels)(
    "$instanceId runs a scheduled prompt after the interval and cancels future runs",
    async (threadModelSelection) => {
      const harness = await createHarness({ testClock: true, threadModelSelection });
  ...
  it("restarts an existing Codex thread on another subscription without reusing its cursor", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("switches provider after a thread is bound and hands over visible context", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("switches provider after the existing thread session has stopped", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
